...
class IDSXMLVehicle(object):
	...
	XML_MAPPINGS = {'CM_PROJECT': 'CM_Project', 'model': 'CM_MODEL', 'type': 'CM_ENGINE_TYPE', 'subtype': 'CM_ENGINE_SUB_TYPE', 'year': 'CM_YEAR_BREAKPOINT', 'code': (code_key, code_value)}
...
class XMLIO(ObjectWrapper):
	XML_ILLEGAL = [chr(a) for a in itertools.chain(itertools.chain(range(0x0, 0x09), range(0xb, 0xd)), range(0xe, 0x20))]
	__proxy = None
...
class IDSContext(object):
	...
	def mnemonics(self):
		if self.__mnemonics == None:
			...
			mnemonics_file = os.path.join(os.path.join(self.__context.root, 'Data'), 'Mnemonics_%s.xml' % (self.__context.lang))
...
def get_references(ctx, obj):
	if isinstance(obj, IDSObject):
		types = [(t, key, t.attributes()[key]) for t in ctx.datatypes().values() for key in t.attributes() if t.attributes()[key].type() == obj.type()]
		...
		for key, attribute in datatype.attributes().items():
			if key in obj_attributes:
				...
				if isinstance(value, list):
					print("%s- %s(%s)%s:" % ('\t'*(tab+1), attribute.name(), attribute.type(), "[]" if attribute.array() else ""))
					for obj in value:
						print_rec(ctx, obj, tab + 2, limit - 1, attribute.type())
				else:
					print("%s- %s(%s)%s: %s" % ('\t'*(tab+1), attribute.name(), attribute.type(), "[]" if attribute.array() else "", object_string(ctx, attribute.function(value), attribute.type())))
			else:
				print("%s- %s(%s)%s: -------" % ('\t'*(tab+1), attribute.name(), attribute.type(), "[]" if attribute.array() else ""))
	elif isinstance(obj, IDSVehicle):
...
def browse(ctx, obj):
	...
	while obj is not None:
		...
		choices = menu(ctx, obj, VEHICLES_KEY, REFERENCES_KEY, MODULES_KEY, PREVIOUS_KEY if len(previous) > 0 else None, NEXT_KEY if len(next) > 0 else None)
